...
-- broadcasts that they need blood, and any hospital can respond.
...
-- Bridge table: a hospital's accept/decline against a donor_request. At
-- most one 'accepted' row per donor_request is enforced by the partial
-- unique index below (mirrors request_matches' UNIQUE(request_id, donor_id)
-- for the one-row-per-pair part).
...
CREATE UNIQUE INDEX uq_donor_request_responses_one_accepted
